...
// broker implements pubsub.Broker using Postgres LISTEN/NOTIFY.
type broker struct {
	db *bun.DB
...
// NewBroker creates a new Postgres pubsub broker using bun.DB.
func NewBroker(db *bun.DB) pubsub.Broker {
	b := &broker{
		db:            db,
...
func (b *broker) multiplex() {
	for {
		channel, payload, err := b.ln.Receive(context.Background())
		if err != nil {
			b.mu.RLock()
			closed := b.closed
			b.mu.RUnlock()
			...
			// Transient error or timeout, prevent tight looping if DB is completely down
...
func (b *broker) Publish(ctx context.Context, channel string, payload []byte) error {
	...
	// pg_notify properly escapes both channel and payload when parameterized
	_, err := b.db.ExecContext(ctx, "SELECT pg_notify(?, ?)", channel, string(payload))
	if err != nil {
...
func (b *broker) Close(ctx context.Context) error {
	...
	b.mu.Lock()
	if err != nil {
		b.closeErr = errors.Newf(errors.ErrCodeUnknown, "failed to close listener: %w", err)
...
func (b *broker) addChannelToSub(sub *subscription, channel string) error {
	...
	needsListen := b.subsByChannel[channel] == nil
	if needsListen {
	...
	if needsListen {
		if err := b.ln.Listen(context.Background(), channel); err != nil {
			b.mu.Lock()
			delete(b.subsByChannel, channel)
			b.mu.Unlock()
			return errors.Newf(errors.ErrCodeUnknown, "failed to listen on channel: %w", err)
		}
	}
...
func (b *broker) removeChannelFromSub(sub *subscription, channel string) error {
	...
	if needsUnlisten {
		if err := b.ln.Unlisten(context.Background(), channel); err != nil {
			return errors.Newf(errors.ErrCodeUnknown, "failed to unlisten from channel: %w", err)
		}
	}
	return nil
}
...
func (b *broker) removeSub(sub *subscription) error {
	...
	var channelsToUnlisten []string
	for channel, subs := range b.subsByChannel {
		delete(subs, sub)
		if len(subs) == 0 {
			delete(b.subsByChannel, channel)
			channelsToUnlisten = append(channelsToUnlisten, channel)
		}
	}
	b.mu.Unlock()

	var firstErr error
	for _, channel := range channelsToUnlisten {
		if err := b.ln.Unlisten(context.Background(), channel); err != nil {
			if firstErr == nil {
				firstErr = errors.Newf(errors.ErrCodeUnknown, "failed to unlisten from channel: %w", err)
			}
		}
	}
	return firstErr
